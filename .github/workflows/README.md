...
Most workflows use the default `GITHUB_TOKEN` with standard permissions. The `pr-validation.yml` workflow requires:
- `pull-requests: write` - To post validation comments
- `issues: write` - PRs are issues, needed for comments
